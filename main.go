...
func createSuperadminAccount(pool *pgxpool.Pool) {
	...
	if username == "" || password == "" || email == "" {
		log.Println("SUPERADMIN_USERNAME, SUPERADMIN_EMAIL or SUPERADMIN_PASSWORD not set, skipping superadmin creation")
		return
	}
	...
	if err != nil {
		log.Printf("Failed to check for existing superadmin: %v", err)
		return
	}
	...
	if exists {
		...
		if err != nil {
			...
			log.Printf("Failed to get superadmin by username: %v", err)
			return
		}
		...
		if user.Email == "" {
			...
			if err != nil {
				log.Printf("Failed to update superadmin email: %v", err)
				return
			}
	...
	if err != nil {
		log.Printf("Failed to create superadmin account: %v", err)
		return
	}
