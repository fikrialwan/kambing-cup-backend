...
func (T *tournamentRepository) Update(ctx context.Context, tournament model.Tournament) error {
	if tournament.ImageUrl == "" {
		_, err := T.pool.Exec(ctx, "UPDATE tournaments SET name = $1, slug = $2, is_show = $3, is_active = $4, total_surah = $5, updated_at = $6 WHERE id = $7", tournament.Name, tournament.Slug, tournament.IsShow, tournament.IsActive, tournament.TotalSurah, time.Now(), tournament.ID)
		return err
	}
